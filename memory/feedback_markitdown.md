...
**🚫 `pdftotext` / `pdfinfo` / `pdftoppm` ARE NOT INSTALLED ON THIS MAC (found 19 Sep 2026).** Poppler is absent entirely, which also means **the Read tool cannot render PDFs** (it shells out to pdftoppm). **Use `markitdown` for every PDF, always.** ⚠️ Crow ran `pdftotext ... 2>/dev/null`, got zero bytes back, and told Korn his FN201 formula sheet was "image-only and needs OCR" — it was fine. **Same failure shape as the 14 Sep AC311 scope call: filled a silence with an inference.** If a extraction comes back empty, check the tool exists before blaming the file.
