...
- **⚡ CANONICAL FOR CAMP DAY (Korn's call, 17 Jul): the ORIGINAL tracker** `1OyGQn4n241XHoFuSAkBEZyvSb_MQtJrDh8UvRIIw-hA` — **real day = 18 groups, เกม 3 = 9 matches of 2v2, each group plays exactly twice** (36 slots / 18 groups — math is exact). This is the sheet that passed the full 15 Jul simulation. **Two gaps before camp:** (1) own-match-bet FILTER auto-block exists ONLY on the rehearsal copy — Korn to decide port vs keep flag-and-catch; (2) its 9 pairings need Klao's real bracket (betting logic keys off group names). Coins re-checked for 18 groups: G3 start 162, sim peak ~195 → 400 ordered still ample. The **rehearsal copy "game score tracker fianl rehersal"** `1W7mJDDbtfpurx_ldKNB1JiY9Ew6ylpyDPJHQ0JkmTtY` (เกม 3 gid 1556510926) was the **rehearsal-day variant (8 groups / 6 matches)** — rebuilt by Crow 16 Jul (arrived values-only, every formula rewritten + mini-sim verified), used at the 16 Jul rehearsal where everything worked; now RETIRED for scoring but keep as the reference implementation of the FILTER auto-block.
