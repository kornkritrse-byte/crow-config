...
- **เกม 3 tab REBUILT 2026-07-15 (Korn's spec, live-tested):** only 3 inputs — match winner A/B (Table 2 col G), bet side + amount (Table 3 grid); everything else auto. Layout: T1 rows 7–25 coin totals+rank+⚠️ status (all 18 groups) · T2 rows 28–37 match results, +2/group flows to T1 · T3 rows 41–61 betting grid 18 groups × 9 matches (ฝั่ง/เหรียญ pairs, cols C–T; U = net P/L) · T4 rows 64+ Top 3 (SORTN tie-safe, spills extra rows on ties) · helper zone rows 120–159 (MAKEARRAY net + rule-check blocks — DO NOT edit/delete; T4 tie-spill can reach row 82, keep 83–119 empty). Rules enforced by ⚠️ flag: own-match bet, stake >4 or <1, side not A/B, negative balance. Pending bets = 0 until winner filled. Uses MAP/LAMBDA/MAKEARRAY. ⚠️ Old conditional formatting/colors may linger from the pre-rebuild layout; input-cell highlighting must be repainted by hand if wanted.
