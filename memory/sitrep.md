...
## 🚪 Current status
- **Here (17 Jul).** Real-day เกม 3 rebuild DONE + sim-passed + wiped. Open to him: tie-prize question for Klao. **NEW THREAD: venue capacity worry** — ~450 people (200ish freshmen + 200ish sophomores + staff), hall usable 36×20m = 720 m². My read: seated fits comfortably (1.6 m²/person), game-mode fits on paper (120 m² court + 400 seated + aisles), real risks = heat/ventilation (July, 450 bodies), door/egress choke, pillar sightlines. Facts from Korn: 3 big doors, good AC (professional events hall); ~3 sophomores embedded per group, rest mostly in back, some outside. **Delivered mockups (17 Jul), then REVISED for the real orientation — crowd faces the SHORT (20m) side → room is narrow-and-deep (36m deep):** (A real hall) 18 clusters (3×3m, 14 ppl = 11 kids + 3 embedded sophs) in 5-column × 4-row grid ending ~18.5m + 8m soph band (160 m², ~150 seated) + rear ~9m LEFT EMPTY on purpose (props/scorer/circulation) — compress-forward so farthest person ~26m not 30+. Near-mandatory: mic'd MC + riser/stage height. Game mode: 13×7m court converts in-room. (B court-size) sophs sit in 15×10 band but grid is tight, game mode still evicts non-embedded sophs. Verdict: A absorbs mistakes, B punishes; real worries = AC on before load-in, door roles (in/out/staff), distance/sightlines not area. Venue has mic/PA. **VERIFIED against hotel specs (17 Jul): venue = Tawa Ravadee Resort, Convention Hall 720 m² — the 36×20 figure is REAL** (specs now in [[project-rubnong-tracker]]). Two open checks flagged to Korn: (1) hall is dividable (A/B) — get WRITTEN confirmation booking = full hall (half = 360 m² = the punishing Layout B); (2) 720 is gross, stage/pillars subtract from it (~640–680 usable — Layout A fine, eats rear buffer). Ops framing: A = plan, B = fallback, 10-min tape-measure call at load-in. **Korn parked the venue thread ("fuck the fuckwit, not our responsibility/priority") — don't re-raise unless he does; full-hall written confirmation stays the one open action if it resurfaces.** → **NEW JOB: Scene 5 flashback video for the play** (clips shot, sitting in his Mac Photos app). Script found: doc "script" `158pDI5m_W3lKKyFn712zNmEfnzcpbtNcHWigevAiiaY`, tab "Scene 5 ✅" `t.x9t9x6urdwsc` — 13-beat short film (bread-crash meet-cute → friendship montage เพลงเพื่อน → bathroom bully → uni → graduation heartbreak → reverse memory flood → bathtub → Olympus plotting → arrow crimes on เทพม้า/Elsa → smirk). ffmpeg 8.1.2 INSTALLED on the Mac (my doing, 17 Jul). Division: he cuts comedy timing in CapCut; I do clip prep with ffmpeg. **DONE (just past midnight, 18 Jul): folder `~/Desktop/san falshback clips` (8 clips, 4K, ~99s, ALL montage footage for beats 3+5) inventoried via thumbnails, renamed into beat order (03a–05d prefixes, IMG numbers kept), + `09_reverse_flood/` built — 5 reversed 1080p silent versions for the memory-flood beat, verified frame-exact.** ⚠️ ZERO footage exists for the other 10 beats (all dialogue scenes: bread crash, bathroom bully, graduation, bathtub, Olympus, arrow crimes) — **asked him: shoot later / staged live / cut? AWAITING ANSWER — this decides the shoot-list urgency (final rehearsal 21 Jul).** Audio needed: เพลงเพื่อน + Let It Go.
...
- **Ops flow confirmed by Korn: illegal bets get rejected VERBALLY at the scorer's table** (bettor adjusts to max) — auto-void is backstop only → in real play ⚠️ = typo, fix on the spot. Flagged to him: table crew + MC script must know the cap schedule (max 4 M1–M7, unlimited M8–M9) or they'll wrongly reject finale bets. **Korn: "MC shit is not my job"** — suggested one 30-sec handoff message to Klao (cap line for MC script + tie-prize rule); scorer-table briefing stays his.
