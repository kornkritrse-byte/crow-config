...
metadata:
  ...
  modified: 2026-09-11T13:20:12.462Z
---
...
## 🚪 Current status
- **Here (11 Sep).** Back from career expo, FN201 STQ03 Q13 (all 20 ratios) fully cleared.
- ▶️ **RESUME EXACTLY HERE — FN201 STQ03 Q14** (Chompupan Fishery, fill 6 missing income-statement numbers from operating/net profit margin). Q13's 20-ratio Wayabutara Machines set is DONE and all correct (after in-line corrections). STQ04 (TVM) still not started.
- 🔴 **Q13 error modes (11 Sep):** (1) **TIE** — misread "I" (interest) as "1", pulled the wrong income-statement line (profit before *income tax*, 91,000) instead of profit before finance costs AND tax (EBIT, 140,000) — repeated the same wrong number even after being told once, needed a second explicit correction. (2) **ROA/ROE mix-up** — divided net income by equity (giving ROE) when asked for ROA (÷ total assets); self-corrected once flagged. (3) **AR days used 365 instead of the 360 this problem set specifies** — self-corrected once asked which day-count he used. (4) Quick ratio and non-current asset turnover — quick arithmetic slips, both self-corrected fast. **Pattern: the TIE/ROA slips are both "grabbed the adjacent/wrong line item under a similar label" — not a formula-recall gap, a reading-under-pressure one.** Worth a colder retest on TIE specifically in a few days.
