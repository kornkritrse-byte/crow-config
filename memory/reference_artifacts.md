...
- **FN201 Obrom's Playbook**: teal `#0D6E78` + amber `#9C5A0B` (semantic), Archivo / Source Serif 4 / IBM Plex Mono. **30 Aug: `.note` re-tinted teal (was red) so the three callouts read distinctly — `.note` teal = worth knowing, `.trap` red = where marks die, `.blank` amber = deck left it blank and I reconstructed it.** New components worth reusing: `.topicbar` (topic divider), `.src`/`.src.deck`/`.src.here` (provenance chips), `.axiom`, `nav.rail li.grp` (nav group labels).
