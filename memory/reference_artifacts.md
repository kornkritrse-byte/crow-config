...
|---|---|---|
| **BA202** Taxation | **The Missed Lectures** — https://claude.ai/code/artifact/2c6630bc-54d1-40a5-b47e-74f69c703483 | L1–L4. Sidebar TOC, 7 SVG diagrams, 25 cold questions, worked examples on all six taxpayer categories. **1 Sep: L4 REBUILT from the 26 Aug class recording** — see the L4 block below. L5–L7 still to add. |
...
| **AC311** Intermediate Accounting I | **Assets Before the Cut-Off** — https://claude.ai/code/artifact/622ff9d5-05d0-498e-8604-ec60013538f4 | Midterm 1 scope (40%). Sessions 1–4 in full from both decks; ~~Provisions was a scaffold only~~ — **REBUILT 15 Sep, see the 🔴 block below.** Sidebar TOC, 6 hand-built SVGs, 8 official-solution walkthroughs, 22 cold questions with hidden answers. **28 Aug (2nd pass, after he confirmed the paper is MCQ + journal entries): added §16 the full 26-entry catalogue tagged `official` vs `derived`, §17 a 10-entry cold drill, §18 the deck's own 4 MCQs.** **3rd pass same day: every journal entry in the doc rebuilt as a real journal page — `Date | Account | Debit | Credit` columns, credits indented, a rule between entries in multi-entry blocks (`.je td.dt`, `tr.ent`). Korn asked for this explicitly — carry the format into any future accounting doc (AC313).** 8 answers are tagged `worked here` = deck slide was blank, no official source. Built 28 Aug. **🔴 15 Sep — §14 PROVISIONS FULLY REBUILT, no longer a scaffold.** Built from TAS 37 + Kieso Ch.13 + the Ch.6 practice solutions + the 14 Sep exam scope + a live drill Korn ran 15 Sep (he got 6 of 7 concepts cold). Contents: plain-English definition then the TAS 37 wording (uncertain timing **OR** amount — the or/and trap) · **a 3-gate decision-tree SVG** (past event → probable >50% → reliable estimate; each NO exits right to nothing/note-only) · legal vs **constructive** obligation · expected value (populations) vs **most likely outcome** (one-offs) with the ฿1.4m-vs-฿2.0m trap named · discounting + reimbursements · **the liability/asset asymmetry table** (asset ladder sits one rung lower — the answer to 'why aren't they treated the same') · the 'contingent assets are never recognised' half-sentence trap · what you can NEVER provide for (future operating losses, own-asset repairs, self-insurance) · onerous contracts (lower of fulfil vs exit) · presentation/disclosure · **7 drills D1–D7** incl. the TAS 37 smoke-filter two-parter, refund policy, single lawsuit, the contingent A/B mirror, the official Ch.6 Problem 5, an onerous lease, and a **10-item rapid classification**. Also: §16 catalogue grew 26→28 entries (I6 refund provision, I7 onerous contract); stale 'not yet taught' labels corrected to 'no deck held' in §01/§15/§16/§20; the adidas gap row closed. ⚠️ Two things are standard-backed but NOT in his practice set — **restructuring provisions** and **discounting a long-dated provision** — flagged in-doc; check her slides first if they surface. New CSS in the family vocabulary: `.plainly` (plain-English statement, display face + accent rule), `.defn` (the standard's formal wording, tinted panel + `<cite>`), `.sub`, `td.c-in/.c-watch/.c-out` (three-state verdict cells), and **`.dg-*`/`.t-*` SVG classes bound to theme tokens** — the older SVGs in this doc still use literal light-theme hex and read poorly in dark; migrate them if ever touched. Verified with Playwright: desktop+phone × light+dark, zero overflow, no duplicate ids, no text escaping its box. |
...
| **AC313** Cost Accounting | **From Raw to Sold** — https://claude.ai/code/artifact/ac9ea735-a4ca-4b8a-898b-c505a5e3ba87 | Built 3 Sep 2026 from `~/Desktop/AC313/` (course package 116pp + Exercise 1 + outline, all via MarkItDown). Midterm 1 scope = sessions 1–7 (Horngren 1,2,4,17,18,19). **10 hand-built SVG money-flow diagrams — Korn asked for these explicitly ("keep the diagrams and flows of money those really help"), carry that into any future doc.** Contents: the master RM→WIP→FG→COGS spine · 4 cost classifications · COGM schedule + T-accounts · job vs process · POHR · **her full 9-entry ABC Corp journal sequence** · under/overapplied with both proration approaches · spoilage/rework/scrap (11 entries) · equivalent units · 5 steps · WA vs FIFO worked both ways · transferred-in · process spoilage · joint products (3 methods) · byproducts (production vs sale) · **all 18 Exercise 1 questions fully worked** · 28 cold-drill questions behind `<details>` · a 25-row trap table. Journal-page format carried over from AC311 per his standing rule. **🔴 15 Sep — NEW §p-run "Running the five steps under exam conditions"** (TOC entry added, sits between §p-five and §p-wa). Written off the two errors Korn actually made in a live drill that night: **(1) one denominator for both cost types**, **(2) step 5 reaching for physical units, or splitting the closing WIP by the RATE ratio instead of each column's own EU.** Contents: the write-it-down-in-this-order procedure · why step 1 earns its place even with no money in it · both errors as `.note trap` blocks · **an SVG showing one pile of 3,000 physical units becoming 3,000 EU of materials AND 750 EU of conversion, recombining only at the end** — the punchline is that the LOWER rate produces the LARGER amount · the full Rayong Ceramics run in the doc's two-table production-report format (12,000 started · 9,000 out · 3,000 closing at 100% DM / 25% CC · DM 96,000 + CC 195,000 → rates 8 and 20 → out 252,000, WIP 39,000, ties to 291,000) · the transfer entry · **4 drills R1–R4**, incl. R2 (same facts but materials added evenly — the percentage belongs to the COST TYPE, not the units) and R4 (spot the error from the tie-back alone, without recomputing). ⚠️ **This doc has no provenance-chip class** (`.src`/`.std` don't exist here) — use italic prose for "worked here" instead. **🔴 16 Sep — NEW §p-fifo "Running FIFO under exam conditions"** (TOC entry added, sits between §p-wa and §p-ti). Built during a live drill after Korn said he was *"confused on which steps to take first"* and asked for a visual. Contents: why FIFO only exists when there IS opening WIP (no BWIP ⇒ both methods identical) · **a 3-group completion-bar SVG with a THREE-STATE encoding — grey = done last period (FIFO ignores), solid = done this period (counted), dashed = not done at all** — the whole EU schedule reads straight off the bars · the FIFO-specific write-it-down order (5 steps, the "split the completed units in two" line is step 2) · **the three errors Korn actually made: (1) started&completed subtracted from the 10,000 STARTED instead of the 9,000 COMPLETED, (2) reverted to the WA count of 12,000 for FIFO DM EU, (3) guessed "5,000" for additional DM on BWIP instead of reading 100% ⇒ zero** · the Saraburi Cement worked run (BWIP 2,000 @100% DM ฿16,000 / 60% CC ฿9,000 · started 10,000 · out 9,000 · EWIP 3,000 @100%/25% · Sept DM ฿90,000 + CC ฿171,000 → EU 10,000/8,550 → rates 9.00/20.00 → out 244,000, EWIP 42,000, ties to 286,000) · **the same data on WA for contrast (out 245,654 / EWIP 40,346 — a ฿1,654 swing)** · **drills F1–F4.** 🔑 **Two checks worth carrying anywhere:** started&completed has TWO independent roads (completed − BWIP, and started − EWIP); and when DM enters at the start, **FIFO DM EU must equal units started** — bigger than that means you counted last month. ⚠️ Favicon set to 🏭 on the 16 Sep republish — keep it. |
| **MK201** Principles of Marketing | **Kotler by Sight** — https://claude.ai/code/artifact/13a35273-eceb-4878-a057-10c10b5bf2d1 | Built 18 Sep 2026 from `~/Desktop/mk/` (Ch1/3/5/7/8 decks + course outline + the 2020 real midterm, all via MarkItDown). **Scope: Ch 1, 3, 5, 7, 8 · exam Sun 20 Sep 13:00–15:00 (2 hrs, per the OUTLINE — the calendar's 13:00–16:00 is the room slot) · 30%.** 🔑 **Design premise: the 2020 real midterm is ~60 SCENARIO MCQs, 5 options, and the distractors are always neighbouring labels** — so every leaf of the tree carries a `.spot` line (the tell that gives the concept away), not just a definition. Structure: 5 chapter "plates" as nested CSS-connector trees · a 33-row **Confusion pairs** table (the boundaries the distractors are built from — revise this last) · **12 scenario drills D1–D12 with hidden answers**. Identity: **Gloock** display + **Spline Sans** body + **Azeret Mono** utility; riso-print palette — printer's blue `#1B4D8F`, crimson `#A5173F`, amber traps `#7E5A07` on a green-biased paper `#F3F4F1` / ink `#171E1B`. Deliberately different from the AC311/AC313 family so the subjects stay distinguishable. ⚠️ **Honesty rails in the doc:** content absent from her decks is flagged in an amber box, not silently included (the textbook's four types of buying decision behavior — complex / dissonance-reducing / habitual / variety-seeking — are NOT in her Ch5 deck); and the endnote states the 2020 answer key is unrecoverable from the PDF export. ⏳ **Not yet done: the hub card still says "not built" and there is no `.uplink` back to the Term 2.1 hub.** |
| EL221 | not built | — |
...
- **AC313 From Raw to Sold** — Bitter / Source Sans 3 / IBM Plex Mono. **Semantic palette, not decorative:** blueprint-blue `#1D4E6F` = mechanism/allocated pool · green `#1B6B4C` = cost that STAYS on the balance sheet (inventoriable) · brick `#A33529` = cost that hits the income statement NOW · ochre `#B07208` = traps. That green/red split is the actual thing the course tests — reuse the idea, not necessarily the hexes.
...
## 📕 BA202 L4 REBUILT FROM THE RECORDING — 1 Sep 2026
Korn dropped `4.BA202-Taxation-Class4-Wed26August2026-Onsite.mp4` (2 h 40 m, 306 MB) into `~/Desktop/BA202 lecture transcripts/` and asked for the new class content added to the artifact. Transcript saved beside L1–L3 as **`L4 - 26 Aug - PIT Part III - Source_Residence Rules + s.40 Eight Categories + s.42 Exemptions.txt`** (21,598 words after collapsing repetition loops).

**Nine sections added / rewritten** (all matched to the doc's existing navy component system — `.rule-card`, `.statute`, `.case`, `.tell`, `.pair-grid`):
`#principles` three economic principles · `#five` her five-element checklist · `#remit` the 2024 remittance change · `#l4-examples` her four worked examples · `#ccc` the Civil & Commercial Code definitions · `#catwork` category-by-category detail · `#mr126` Ministerial Regulation 126.

### 🔴 The examinable findings from the recording
- **2024 remittance change — CONFIRMED** (the doc had it at 70% confidence off a blank slide title). Pre-2024 foreign income was taxed only if remitted **in the same tax year**; from **Jan 2024** a **Director-General announcement** — the statute was never amended — makes it taxable in the **year of remittance, whatever year it was earned**. She says she disagrees with the method and that it is constitutionally contested. **Old past papers will give the wrong answer.**
- **She names her own exam trap:** ask *globally* and there are **three** connecting-factor rules (source / residence / citizenship); ask about **Thailand** and the answer is **two** — citizenship is US-only.
- **180-day rule CONFIRMED** — aggregate, assessed **per tax year**, nationality and visa irrelevant.
- **MR126 is handed out WITH the exam paper** — do not spend cheat-sheet space on it; **s.42 goes on the sheet**. The two clauses she named: **cl. 23** (capital gain on shares sold **through the SET** — exempt) and **cl. 85** (ASEAN exchanges). ⚠️ **Sold OUTSIDE an exchange → taxable, category 40(4).**
- **Category 5 breach mechanic:** hire-purchase / instalment sale with **no breach → 40(8)**; **with breach → 40(5)**. The word "breach" is what moves the category.
- **40(4) is the only category with NO deduction.** Capital gain = the **gain only** (buy 100, sell 150 → include 50); a loss on ordinary shares is **not deductible**.
- **40(2) vs 40(7)** turns on **who supplies the materials** (tools alone = 2; materials too = 7). Most real construction contracts are **7**.
- **Underlying CCC sections she teaches:** `s.575` employment (heart = **controlling power**), `s.587` hire of work (heart = **no** controlling power), `s.537` rental, `s.572` hire-purchase.
- **Crypto: she deferred it twice** — own regulation, time-limited exemptions. The doc's crypto block is the handout's account, now flagged as such.
- **Mr A is the spine of the midterm** — by end of L4 he holds income in five categories at once.

⚠️ **Two transcription cautions recorded in the doc:** she repeatedly says "section 40" while walking the **s.41** checklist, and the CCC section numbers come from speech, not a slide.
