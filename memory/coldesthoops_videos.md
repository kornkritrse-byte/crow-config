---
name: coldesthoops-videos
description: Running log of all coldesthoops YouTube Shorts topics already published — check this before suggesting new topics
metadata: 
  node_type: memory
  type: project
  originSessionId: 92b74bf0-12bc-4ea0-b490-ec703e2fac82
---

# coldesthoops — Published Videos

Check this before suggesting any new topic. Never repeat a topic from this list.

| # | Topic | Notes |
|---|-------|-------|
| 1 | Game-Winners / Buzzer Beaters | First upload |
| 2 | Top 5 Coldest Crossovers | Second upload, 2026-06-05 |
| 3 | Ankle Breakers | — |
| 4 | Most Disrespectful Dunks | — |
| 5 | Most Embarrassing Blocks | Mutombo, Ibaka, Gobert, Giannis, DeAndre Jordan |
| 6 | Craziest Alley-Oops | Blake Griffin, DeAndre Jordan, Zion, Ja Morant, LeBron James |
| 7 | Top 5 Most Unguardable Moves | Kyrie, Duncan, Dirk, Hakeem, Kareem — built 2026-06-08 |
| 8 | Most Disrespectful Celebrations | Steph shimmy, Reggie Miller choke sign, Shaq on Dudley, LeBron silencer, AI step-over Lue — built 2026-06-08 |

**Why:** Avoid suggesting the same topic twice — wastes his time having to correct it.

**How to apply:** Before generating topic suggestions for coldesthoops, read this file and exclude any topics that overlap or are too similar to entries above. Update this list every time a new video is confirmed published.
